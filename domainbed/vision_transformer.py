...
sys.path = ["./domainbed"] + sys.path

from vit_helpers import build_model_with_cfg, named_apply, adapt_input_conv
from timm.models.layers import PatchEmbed, Mlp, DropPath, trunc_normal_, lecun_normal_
from timm.models.registry import register_model
from mixture_of_experts import OracleMoE, HierarchicalMoE, AllExperts, HierarchicalClassExperts
...
class AllExpertsBlock(nn.Module):
    ...
    def forward(self, x, return_attention=False):
        # print("curr_layer :", self.cur_layer)
        # print("x shape in allexpert block:", x.shape)
        if return_attention:
            ...
            if hasattr(self.mlp, 'experts'):
                ...
                if isinstance(tmp, tuple):
                    ...
                    x_temp = tmp
                    # print("f :", x.unsqueeze(dim=2).shape)
                    # print("s :", self.drop_path(x_temp).shape)
                    # print("t :", x_temp.shape)
                    x = x.unsqueeze(dim=2) + self.drop_path(x_temp)
                    return x
            elif hasattr(self.mlp, 'fc1'):
                x_temp = self.mlp(self.norm2(x))
            else:
                raise Exception
            # print("x_temp in allexpert block:", x_temp.shape)
...
class HierarchicalBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, cur_depth=0, moe_layers=None, num_experts=4, index_hook=False):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop)
        # NOTE: drop path for stochastic depth, we shall see if this is better than dropout here
        ...
        self.cur_layer = moe_layers[cur_depth] if moe_layers is not None else 'F'
        if self.cur_layer == 'S':
            self.mlp = HierarchicalClassExperts(
                dim=dim,
                num_experts=num_experts,  # increase the experts (# parameters) of your model without increasing computation
                hidden_dim=mlp_hidden_dim,  # size of hidden dimension in each expert, defaults to 4 * dimension
                activation=act_layer,  # use your preferred activation, will default to GELU
...
class AllExpertsVisionTransformer(nn.Module):
    """ Vision Transformer that yields output of all the experts
        Single token is passed through all experts
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        assert moe_layers[-1] == 'S', "Last layer is not sparse"
        self.block_list = [AllExpertsBlock(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                    cur_depth=i, moe_layers=moe_layers, num_experts=num_experts, Hierachical=Hierachical,
                    index_hook=index_hook)
                for i in range(depth)]
        if sparse2:
            if moe_layers[-4] == 'S':
                self.block_list[-4] = Block(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                        attn_drop=attn_drop_rate, drop_path=dpr[-4], norm_layer=norm_layer, act_layer=act_layer,
                        cur_depth=len(moe_layers)-3, moe_layers=moe_layers, num_experts=num_experts)
    ...
    def forward(self, x):
        # print("Input shape to vision transformers :", x)
...
class OracleVisionTransformer(nn.Module):
    """ Vision Transformer
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.ModuleList([
            OracleBlock(
                dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                cur_depth=i, moe_layers=moe_layers, num_experts=num_experts, index_hook=index_hook)
            for i in range(depth)])
        self.norm = norm_layer(embed_dim)
...
class HierarchicalVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, depth)]  # stochastic depth decay rule
        assert moe_layers[-1] == 'S', "Last layer is not sparse"
        self.block_list = [HierarchicalBlock(
                    dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, drop=drop_rate,
                    attn_drop=attn_drop_rate, drop_path=dpr[i], norm_layer=norm_layer, act_layer=act_layer,
                    cur_depth=i, moe_layers=moe_layers, num_experts=num_experts, index_hook=index_hook)
                for i in range(depth)]
    ...
    def forward_features(self, x):
        ...
        if self.dist_token is None:
            ...
            raise Exception("Distillation token not implemented")
...
class SkipVisionTransformer(nn.Module):
    """ Vision Transformer that yields output of all the experts
        Single token is passed through all experts
    """
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.blocks = nn.Sequential(*(self.block_list[:-2]))
        self.norm = norm_layer(embed_dim)
        self.gatings = []
    ...
    def forward_features(self, x):
        ...
        x = self.pos_drop(x + self.pos_embed)
        x = self.blocks(x)
        self.block_list[-2].to(x.device)
        self.block_list[-1].to(x.device)
        x_passed = self.block_list[-2](x)
        x = self.block_list[-1]((x+x_passed)/2) 
        x = self.norm(x)
...
def _create_all_exp_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(
        AllExpertsVisionTransformer, variant, pretrained,
...
def _create_oracle_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    model = build_model_with_cfg(
        OracleVisionTransformer, variant, pretrained,
        default_cfg=default_cfg,
        representation_size=repr_size,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in default_cfg['url'],
...
def _create_hierarchical_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    print("kwargs", kwargs)
    model = build_model_with_cfg(
        HierarchicalVisionTransformer, variant, pretrained,
        default_cfg=default_cfg,
        representation_size=repr_size,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in default_cfg['url'],
...
def _create_skip_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    default_cfg = default_cfg or default_cfgs[variant]
    if kwargs.get('features_only', None):
        raise RuntimeError('features_only not implemented for Vision Transformer models.')
    ...
    print("kwargs", kwargs)
    model = build_model_with_cfg(
        SkipVisionTransformer, variant, pretrained,
        default_cfg=default_cfg,
        representation_size=repr_size,
        pretrained_filter_fn=checkpoint_filter_fn,
        pretrained_custom_load='npz' in default_cfg['url'],
...
@register_model
def oracle_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_oracle_vision_transformer('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def all_exp_deit_small_distilled_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small distilled model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_all_exp_vision_transformer(
        'deit_small_distilled_patch16_224', pretrained=pretrained, distilled=True, **model_kwargs)
    return model

@register_model
def hierarchical_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_hierarchical_vision_transformer('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def skip_deit_small_patch16_224(pretrained=False, **kwargs):
    """ DeiT-small model @ 224x224 from paper (https://arxiv.org/abs/2012.12877).
    ImageNet-1k weights from https://github.com/facebookresearch/deit.
    """
    model_kwargs = dict(patch_size=16, embed_dim=384, depth=12, num_heads=6, **kwargs)
    model = _create_skip_vision_transformer('deit_small_patch16_224', pretrained=pretrained, **model_kwargs)
    return model
