...
USE_COMBINED_ROUTING = os.getenv("USE_COMBINED_ROUTING", default=False)

def plot_loss(algo, args):
    # print("CE loss :",algo.cross_entropy_loss_list)
    # print("KL div loss :", algo.kl_div_loss_list)
    x = list(range(len(algo.cross_entropy_loss_list)))
    plt.plot(x, algo.cross_entropy_loss_list, label="Cross-Entropy loss")
    plt.plot(x, algo.kl_div_loss_list, label="KL-Div loss")
    plt.legend()
    plt.savefig(f"plots/algo_{args.algorithm}_ds_{args.dataset}_lossplot.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--rand_routing', action='store_true')
    ...
    random_routing = os.getenv("USE_RANDOM_ROUTING", default=False) or args.rand_routing
    #################################################################################

    config.TEST_ENV = args.test_envs[0]
    if USE_DOMAIN_CLASS_ROUTING:
        if USE_COMBINED_ROUTING:
            filename = f"domain_class_routings/{args.dataset}.pkl"
        else:
            filename = f"domain_class_routings/{args.dataset}_{args.test_envs[0]}.pkl"
        with open(filename, 'rb') as handle:
            config.DOMAIN_CLASS_DICT = pickle.load(handle)
            print("Loaded routing : ", config.DOMAIN_CLASS_DICT)
    ...
    if not random_routing:
        ...
        if args.load_routing:
            ...
            config.ROUTING = torch.load(f"routings/{typ}_{args.dataset}_te{args.test_envs[0]}.pt").cuda()
    ...
    if random_routing:
        config.ROUTING = torch.randint(low=0, high=config.NUM_EXPERTS, size=(config.NUM_CLASSES,1)).squeeze().cuda()
        print(config.ROUTING)
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for name, loader, weights in evals:
                ...
                if args.algorithm == "ConfidentExperts":
                    acc = misc.accuracy(algorithm, loader, weights, device)
    ...
    if args.algorithm == "ConfidentExperts":
        plot_loss(algorithm, args)
    model_path = f'model_alg{args.algorithm}_ds{args.dataset}_te{args.test_envs[0]}_texp{config.NUM_EXPERTS}_depth{len(config.MOE_LAYERS)}.pkl'
